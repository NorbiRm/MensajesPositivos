...
public class MainActivity extends AppCompatActivity implements CompoundButton.OnCheckedChangeListener {
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        day = (Switch)findViewById(R.id.sDay);
        linearLayout = (ConstraintLayout)findViewById(R.id.viewPrin);
        editText = (EditText) findViewById(R.id.editText);

        day.setOnCheckedChangeListener(this);
        indexMsg = rand.nextInt(mensajes.length);
        textView.setText(mensajes[indexMsg]);
    ...
    public void onClickAnterior(View view){
        if(indexMsg>0){
            ...
            indexMsg=mensajes.length-1;
        }
        textView.setText(mensajes[indexMsg]);
    }
    public void onClickSiguiente(View view){
        if(indexMsg<(mensajes.length-1)){
            indexMsg++;
    ...
    public void onCheckedChanged(CompoundButton compoundButton, boolean b){
        if(day.isChecked()){
            linearLayout.setBackgroundResource(R.drawable.sadasd);
        }else{
            linearLayout.setBackgroundResource(R.drawable.amsterdam280);
        }
    }
